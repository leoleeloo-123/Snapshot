...
const AppContent: React.FC = () => {
  ...
  const [selectedLoanId, setSelectedLoanId] = useState<number | null | undefined>(undefined);
  ...
  const renderModule = () => {
    ...
    if (selectedLoanId !== undefined) {
      return (
        <LoanDetail 
          ...
          onBack={() => setSelectedLoanId(undefined)} 
        />
      );
    }
    ...
    switch (activeModule) {
      ...
      case 'accounts': return <PortfolioList onSelectAccount={(id) => setSelectedAccountId(id)} onSelectAsset={(id) => setSelectedAssetId(id)} onSelectLoan={(id) => setSelectedLoanId(id)} />;
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-[100dvh] bg-[var(--bg-primary)]">
